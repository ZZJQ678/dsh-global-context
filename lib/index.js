 ...
 *
 * 职责有两件：
 *   1. 把「全局提示词」文本注入到**每个会话系统提示词的最顶部**（排在内置身份之前）。
 *   2. 在 DSH Web 服务上开一个精确路由，让浏览器里的「全局提示词配置」标签页
 *      可以读取和保存这段文本。
 *
...
/** 浏览器半边用它上报标签页定位结果（只做诊断，不写正文）。 */
export const PLACEMENT_ROUTE = '/api/dsh-global-context/placement';
...
export function apply(ctx, config = {}) {
  ...
  let routeError = null;
  let placementRouteRegistered = false;
  /** 浏览器半边上报的标签页定位结果（诊断用）。 */
  let placement = null;
  ...
  const registerRoute = (webServer) => {
    try {
      ...
      // 诊断路由：浏览器半边把「标签页排到第几位」报上来，写进状态文件，便于事后核对。
      webServer.register({
        kind: 'exact',
        path: PLACEMENT_ROUTE,
        handler: async (req, res) => {
          try {
            if (!sameOrigin(req)) {
              sendJson(res, 403, { ok: false, error: '仅允许同源请求' });
            ...
            if (String(req.method ?? 'GET').toUpperCase() === 'GET') {
              sendJson(res, 200, { ok: true, placement });
              return;
            }
            const body = await readBody(req, MAX_BODY_BYTES);
            const parsed = JSON.parse(body === '' ? '{}' : body);
            placement = {
              order: Number.isFinite(parsed?.order) ? parsed.order : null,
              smart: parsed?.smart === true,
              rank: Number.isFinite(parsed?.rank) ? parsed.rank : null,
              others: Array.isArray(parsed?.others) ? parsed.others.slice(0, 32) : null,
              failed: typeof parsed?.failed === 'string' ? parsed.failed.slice(0, 200) : null,
            };
            report();
            sendJson(res, 200, { ok: true, placement });
          } catch (error) {
