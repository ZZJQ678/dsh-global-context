...
function writeStatus(status) {
  try {
    writeFileSync(path.join(resolveHome(), STATUS_FILE_NAME), `${JSON.stringify({ ...status, at: new Date().toISOString() }, null, 2)}\n`, 'utf8');
  } catch {
    /* 诊断用途，失败不影响宿主。 */
  }
}
...
export function apply(ctx, config = {}) {
  ...
  try {
    ...
    writeStatus({ ok: sectionRegistered, sectionName, order, enabled, textPath, bytes, routeRegistered, sectionError, routeError });
  } catch {
    /* 诊断输出失败不影响宿主。 */
  }
}
