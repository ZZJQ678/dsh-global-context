 ...
 *
 * 它往对话视图的 `conversation.view` 插槽注册一个标签页「全局提示词配置」。
 ...
 * 标签页通过宿主暴露的 /api/dsh-global-context 读写全局提示词文本，
      ...
      const save = React.useCallback(
        async (value) => {
          ...
          try {
            ...
            setNote({
              kind: 'ok',
              text: value === '' ? '已清空：全局提示词不再注入（下一条消息起生效）' : `已保存 ${data.bytes} 字节，下一条消息即生效`,
            })
            await load()
          } catch (error) {
