...
2. **在对话视图新增一个标签页。** 原装 DSH 的对话视图只有「对话」一个标签页，「轨迹」「上下文」
   等是后续装插件才有的；本插件新增的「全局上下文配置」会自动排在已有标签页之后，
   里面是一个可以直接**编辑、保存、清空**的文本框。
...
本包只通过 GitHub 分发（没有发布到 npm），上面就是完整的安装命令。
...
搜这两个词都能搜到；进入插件详情页，正文就是本文件 —— 也就是**中文**。
